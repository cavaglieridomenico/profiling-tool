...
export const getAdbPath = (): string => {
  ...
  if (process.env.ADB_PATH) {
    logger.info(`Using adb from ADB_PATH: ${process.env.ADB_PATH}`);
  ...
  if (process.env.ANDROID_HOME) {
    ...
    logger.info(`Using adb from ANDROID_HOME: ${adbPathResult}`);
    return adbPathResult;
  }
  // 3. Fallback to assuming 'adb' is in the system's PATH
  logger.info("Assuming 'adb' is in the system's PATH.");
  return 'adb';
};
...
export function standardConnection() {
  ...
  logger.info('🔌 STARTING STANDARD ADB CONNECTION...');

  try {
    // STEP 1: Check ADB Availability and Device Status
    logger.info('[1/3] Checking connected devices...');
    ...
    logger.success(`Device detected: ${deviceLines[0].split('\t')[0]}`);

    // STEP 2: Clean and Set Port Forwarding
    logger.info('[2/3] Configuring Port Forwarding (9222)...');
    ...
    // STEP 3: Verification
    logger.info('[3/3] Verifying connection integrity...');
    ...
    if ((error as any).stderr) {
      logger.error(`ADB Output: ${(error as any).stderr.toString()}`);
    }
...
export async function initializeBrowser(mode: string): Promise<Browser> {
  ...
  if (mode === 'mobile') {
    logger.info('Connecting to mobile device...');
    const browserWSEndpoint = await getMobileWebSocketEndpoint();
    logger.info(`WebSocket endpoint: ${browserWSEndpoint}`);
    ...
    logger.info('Launching desktop Chrome...');
    ...
    logger.error('Invalid mode specified. Use "mobile" or "desktop".');
    process.exit(1);
  }
