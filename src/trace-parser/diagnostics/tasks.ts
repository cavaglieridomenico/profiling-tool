...
console.log(`Analyzing CrRendererMain (PID: ${pid}, TID: ${tid}) in ${filePath}`);

const tasks = events.filter((e: any) => 
    e.pid === pid && 
    e.tid === tid && 
    (e.name === 'RunTask' || e.name === 'ThreadPool_RunTask' || e.name === 'ThreadControllerImpl::RunTask') &&
    e.dur
);
...
longTasks.forEach((t: any, i: number) => {
    console.log(`${i + 1}. ${t.name} - Duration: ${t.durMs.toFixed(2)}ms - TS: ${t.ts}`);
});
