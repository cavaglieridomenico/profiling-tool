...
async function main() {
    ...
    // Header
    csvLines.push(',Long tasks > 100 ms,Long tasks > 500 ms,Longest task (ms),JS heap min (MB),JS heap max (MB),INP,CLS,DevTools issues');
    ...
    let workerCount = 1;
    for (const [key, threadMetrics] of threadEntries) {
    ...
    console.log(`Metrics extracted and saved to: ${outputPath}`);
}
